...
async def web_search(query: str, max_results: int = 5, timeout: int = 10) -> str:
    """
    ...
    Args:
        ...
        timeout: Maximum seconds to wait for search (default 10)
    ...
    try:
        ...
        result = await asyncio.wait_for(
            client.search_and_format(query, max_results),
            timeout=timeout
        )
        return result
    except asyncio.TimeoutError:
        print(f"[WebSearch] Timeout after {timeout}s - DuckDuckGo may be slow or blocked")
        return "Web search timed out. DuckDuckGo search is currently unavailable."
    except Exception as e:
